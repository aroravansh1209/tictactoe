...
import Vanish from "../assets/vanish.mp3"
import Win from "../assets/win.mp3"
import Power from "../assets/powerup.mp3"
...
export default function BlinkTacToe() {
    ...
    const [powerUpGiven, setPowerUpGiven] = useState([false, false])
    ...
    // Sound effects
    const [playPlace] = useSound(Place, { soundEnabled })
    const [playVanish] = useSound(Vanish, { soundEnabled })
    const [playWin] = useSound(Win, { soundEnabled })
    const [playPowerUp] = useSound(Power, { soundEnabled })
    const [playError] = useSound(Error, { soundEnabled })
    const [playClick] = useSound(Click, { soundEnabled })
    ...
    useEffect(() => {
        if (
            ...
            // Mark this player as having received a power-up
            setPowerUpGiven((prev) => {
                const updated = [...prev]
                updated[currentPlayer - 1] = true
                return updated
            })
        }
    }, [turnCount, gameStarted, powerUp, playPowerUp, winner, currentPlayer, powerUpGiven])
                ...
                <div className="w-full">
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        ...
                        <div className="flex gap-2 flex-wrap justify-center">
                            ...
                            <button
                                className={`p-2 rounded-md border border-gray-300 hover:bg-gray-100 transition-colors ${musicEnabled ? "bg-purple-100" : ""
                                    }`}
